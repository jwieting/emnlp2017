...
class lasagne_gran_layer(lasagne.layers.MergeLayer):
    ...
    def __init__(self, incoming, num_units,
        ...
        super(lasagne_gran_layer, self).__init__(incomings, **kwargs)
        ...
        if self.gran_type == 3 or self.gran_type == 4 or self.gran_type == 5:
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step(input_n, cell_previous, hid_previous, avg_previous, *args):
            ...
            elif self.gran_type == 6:
                avg = nonlinearities.tanh(avg_input)
            elif self.gran_type == 7:
                avg_input = T.dot(x, self.W_avg1) * T.dot(hid, self.W_avg2) + self.b_avg
                avg = x * nonlinearities.sigmoid(avg_input)
            elif self.gran_type == 2:
                avg = hid*nonlinearities.sigmoid(avg_input)
            elif self.gran_type == 3:
        ...
        if self.gran_type == 3 or self.gran_type == 5:
